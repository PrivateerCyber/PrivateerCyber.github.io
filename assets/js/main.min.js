...
function initFormValidation(){const contactForm=document.getElementById('contact-form');if(!contactForm)return;const formFields=contactForm.querySelectorAll('.form-control');formFields.forEach(field=>{field.addEventListener('input',()=>validateField(field));field.addEventListener('blur',()=>validateField(field,!0))});contactForm.addEventListener('submit',function(e){e.preventDefault();if(contactForm.classList.contains('submitting')){return}
let formValid=!0;formFields.forEach(field=>{if(!validateField(field,!0)){formValid=!1}});if(formValid){const submitBtn=contactForm.querySelector('.submit-btn');const originalBtnText=submitBtn.innerHTML;contactForm.classList.add('submitting');submitBtn.disabled=!0;submitBtn.innerHTML='<i class="fas fa-spinner fa-spin"></i> '+(window.location.pathname.includes('/fr/')?'Envoi en cours...':'Sending...');const formData=new FormData(contactForm);fetch(contactForm.action,{method:'POST',body:formData,headers:{'Accept':'application/json'}}).then(response=>{if(!response.ok){throw new Error('Network response was not ok')}
return response.json()}).then(data=>{if(data.success){showFormStatus('success',window.location.pathname.includes('/fr/')?'Message envoyé avec succès!':'Message sent successfully!');contactForm.reset()}else{showFormStatus('error',data.message||(window.location.pathname.includes('/fr/')?'Échec de l\'envoi du message. Veuillez réessayer.':'Failed to send message. Please try again.'))}}).catch(error=>{showFormStatus('error',window.location.pathname.includes('/fr/')?'Une erreur s\'est produite. Veuillez réessayer plus tard.':'An error occurred. Please try again later.')}).finally(()=>{contactForm.classList.remove('submitting');submitBtn.disabled=!1;submitBtn.innerHTML=originalBtnText})}})}
function validateField(field,showError=!1){const value=field.value.trim();const isRequired=field.hasAttribute('required');const fieldType=field.getAttribute('type');const pattern=field.getAttribute('pattern');let isValid=!0;let errorMessage='';if(isRequired&&value===''){isValid=!1;errorMessage=window.location.pathname.includes('/fr/')?'Ce champ est requis':'This field is required'}else if(fieldType==='email'&&value!==''&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)){isValid=!1;errorMessage=window.location.pathname.includes('/fr/')?'Veuillez entrer une adresse email valide':'Please enter a valid email address'}else if(pattern&&value!==''&&!new RegExp(pattern).test(value)){isValid=!1;errorMessage=window.location.pathname.includes('/fr/')?'Veuillez entrer une valeur valide':'Please enter a valid value'}
const errorElement=field.parentElement.querySelector('.error-message');if(errorElement){if(!isValid&&showError){errorElement.textContent=errorMessage;errorElement.classList.add('visible');field.classList.add('error');field.setAttribute('aria-invalid','true')}else{errorElement.textContent='';errorElement.classList.remove('visible');field.classList.remove('error');field.setAttribute('aria-invalid','false')}}
return isValid}
function showFormStatus(type,message){const statusElement=document.querySelector('.form-status');if(!statusElement)return;statusElement.textContent=message;statusElement.className='form-status '+type;setTimeout(()=>{statusElement.textContent='';statusElement.className='form-status'},5000)}
