...
function initHeroVideo(){const heroVideo=document.querySelector('.hero-video');if(heroVideo){heroVideo.addEventListener('loadeddata',()=>{heroVideo.classList.add('loaded')});heroVideo.addEventListener('error',()=>{heroVideo.classList.add('error');const fallbackImg=document.querySelector('.hero img[aria-hidden="true"]');if(fallbackImg){fallbackImg.style.display='block';heroVideo.style.display='none'}});const videoDuration=22;const fadeDuration=1.5;const baseOpacity=0.3;const fadeOutStart=videoDuration-fadeDuration;setInterval(()=>{const currentTime=heroVideo.currentTime;if(currentTime>=fadeOutStart){const timeLeft=videoDuration-currentTime;const fadeProgress=timeLeft/fadeDuration;heroVideo.style.opacity=String(baseOpacity*fadeProgress)}else if(currentTime<=fadeDuration){const fadeProgress=currentTime/fadeDuration;heroVideo.style.opacity=String(baseOpacity*fadeProgress)}else{heroVideo.style.opacity=String(baseOpacity)}},100)}}
